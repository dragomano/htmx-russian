...
* `scrollBehavior:` `'instant'` — строка: поведение прокрутки при использовании модификатора [show](/attributes/hx-swap/#%D0%BF%D1%80%D0%BE%D0%BA%D1%80%D1%83%D1%82%D0%BA%D0%B0-scroll-%D0%B8-show) с `hx-swap`. Допустимые значения: `instant` (прокрутка должна происходить мгновенно в одном прыжке), `smooth` (прокрутка должна анимироваться плавно) и `auto` (поведение прокрутки определяется вычисленным значением [scroll-behavior](https://developer.mozilla.org/ru/docs/Web/CSS/scroll-behavior)).
* `defaultFocusScroll:` `false` — булевое значение: должен ли фокусированный элемент прокручиваться в видимую область, может быть переопределено с помощью модификатора замены [focus-scroll](/attributes/hx-swap/#%D0%BF%D1%80%D0%BE%D0%BA%D1%80%D1%83%D1%82%D0%BA%D0%B0-%D0%BA-%D1%84%D0%BE%D0%BA%D1%83%D1%81%D1%83).
...
* `htmx.config.responseHandling:` `[...]` — HtmxResponseHandlingConfig[]: поведение [обработки ответов](/response-handling) по умолчанию для кодов состояния ответа можно настроить здесь, чтобы либо заменить, либо выдать ошибку.
* `htmx.config.allowNestedOobSwaps:` `true` — булевое значение: разрешать ли обработку OOB замен на элементах, которые вложены в основной элемент ответа. См. [Вложенные OOB замены](/attributes/hx-swap-oob/#nested-oob-swaps).
